...
- [ ] Step 2: Run the route test and observe the expected failure.
...
- [ ] Step 3: Add the four guides and reduce the public entry points.
...
- [ ] Step 4: Run the route test and documentation link checks.
...
- [ ] Step 3: Implement the minimal additive protocol and projection.
...
- [ ] Step 4: Re-run the focused tests and the repository status regression suite.
...
- [ ] Step 5: Commit the B projection slice.
...
- [ ] Step 1: Write failing stale-query, gate-consistency, and transport-parity tests.
...
- [ ] Step 3: Implement shared admission without replacing existing lifecycle validators.
...
- [ ] Step 4: Make CLI and MCP expose the same structured result.
...
- [ ] Step 5: Run focused red-green tests and existing parity tests.
...
- [ ] Step 6: Commit the shared-admission slice.
...
- [ ] Step 1: Add failing generator and invalid-example tests.
...
- [ ] Step 3: Extend existing capability projection and generator markers.
...
- [ ] Step 4: Generate and validate the mechanical regions.
...
- [ ] Step 1: Add failing projection and delivery assertions.
...
- [ ] Step 2: Run the focused tests and observe the expected failure.
    ...
    Expected: the new idempotence/write-failure or delivery assertions fail before the bounded projection/output changes.

- [ ] Step 3: Implement the smallest bounded projection change.

    Keep planned_changes restricted to the explicit derived Work Item documents and parity rows. Bind the result to the terminal evidence identity and expose a deterministic projection input/output summary. Catch write failures at the command boundary as projection_pending without writing a successor or changing close. Do not change validate_terminal_evidence rules or any historical record. Only change outcome_render.rs if the failing delivery test demonstrates that a required status, risk, evidence, decision, or next-action domain is absent.

- [ ] Step 4: Re-run projection and delivery tests.
...
- [ ] Step 5: Commit the C slice.
    ...
    git commit -m "test: bound projection and outcome delivery"
...
- [ ] Step 1: Write the failing measurement contract.
...
- [ ] Step 2: Run the measurement test and record the pre-change failure if applicable.
...
- [ ] Step 3: Record final metrics and scope-separated limitations.
...
- [ ] Step 4: Run cheap checks before formal verification.
