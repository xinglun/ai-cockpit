...
| WI-170 — v0.2.20 post-release parity と branch reconciliation | In progress | この Work Item は release 後の documentation correction と、検証済み merged branch の正確な cleanup proof を記録し、次の比較 batch に進む前の基線を整えます。 |
| WI-171 — finalization reconciliation successor | In progress | この successor は WI-170 の immutable な recovery decision を close 前に不足していた resource-finalization boundary に bind します。 |
