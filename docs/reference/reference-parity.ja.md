...
| WI-208 — release tag finalization order（current） | Implemented | [Work Item](../work-items/WI-208-release-tag-pending-close-finalization.ja.md)；`.ai/evidence/WI-208-release-tag-pending-close-finalization.verification.json`；`.ai/decisions/WI-208-release-tag-pending-close-finalization.finalize.json`；PR #158 の正確な context を verification 前に bind し、merge-close 待ちとして保持します。 |
