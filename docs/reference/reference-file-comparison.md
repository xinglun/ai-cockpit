...
- Rust comparison baseline: [`xinglun/ai-cockpit`](https://github.com/xinglun/ai-cockpit) `origin/main` at `446ec82c0e8c0b55094c4654f9e6a098bb75f179`.
- Runtime used for the comparison work: the reviewed `ai-cockpit v0.2.90` binary, SHA256 `sha256:4ef144434dc27c40a8b6d0de7988fb1bc914fa10a9d2b7a2b7b0b9ed036f957c`.
...
The reviewed Runtime is v0.2.90 with binary digest
...
WI-612 re-read the next twenty maintained source test paths at the pinned local
reference commit `fde3380f81fea5fd2e288f7a8849f737dc074060`, one file at a time.
...
## WI-629 — reference rebaseline batch 51 (first 60 changed paths)

WI-629 re-read the first 60 non-history paths changed at pinned source commit
`a9224aed77b5c317b53c4551a9eec306d91ee330`. Fifty-eight paths are
`implemented-different-by-design`; the source adopter capability manifest and
schema are `reference-only` because the Rust Runtime intentionally exposes
truthful request-scoped capability/status views rather than the source
installer manifest or its JSON wire format. No `migrate-gap` was found.

The complete per-file decisions are recorded below and in the [WI-629 Work Item](../work-items/WI-629-reference-rebaseline-batch-51.md).
...
WI-640 re-read the next sixty deferred paths one by one at the pinned local
reference commit `a9224aed77b5c317b53c4551a9eec306d91ee330`. Fifty-four paths are
`implemented-different-by-design`; six source-specific fixtures or historical
records are `reference-only`. No path remains `deferred-next-batch` or
`migrate-gap` in this bounded batch. Records whose earlier ledger had no source
change marker explicitly record `sourceChangedSincePrevious=false`; this means
no prior source-digest claim existed, not that the path was skipped.

The exact file-level decisions are authoritative in
[`reference_file_inventory.json`](../../tests/conformance/reference_file_inventory.json):

| Pinned reference path | Classification | Rust counterpart / bounded decision |
| --- | --- | --- |
| `tests/test_quality_measurements.py` | implemented-different-by-design | `crates/cockpit-verification/tests/cost_observation.rs; crates/cockpit-cli/tests/performance.rs; crates/cockpit-repository/tests/ci_quality_gate.rs; docs/reference/performance-diagnosis.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_quality_scope.py` | implemented-different-by-design | `crates/cockpit-verification/tests/policy_planner.rs; crates/cockpit-verification/tests/route_convergence.rs; tests/ci/quality_route.py; docs/reference/governance-profiles.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_quality_session.py` | implemented-different-by-design | `tests/ci/quality_route.py; tests/ci/quality_route_test.py; .github/workflows/ci.yml; docs/reference/ci-quality-gates.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_quality_shard_workspace.py` | implemented-different-by-design | `crates/cockpit-verification/tests/execution.rs; tests/release/adopter_acceptance.sh; tests/release/adopter_upgrade_acceptance.sh; docs/reference/ci-quality-gates.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_quality_telemetry.py` | implemented-different-by-design | `crates/cockpit-verification/tests/cost_observation.rs; crates/cockpit-cli/tests/performance.rs; tests/ci/run_repository_gates.py; docs/reference/performance-diagnosis.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_quality_test_manifest.py` | implemented-different-by-design | `tests/ci/repository_gate_manifest.json; tests/ci/repository_gate_manifest_test.py; crates/cockpit-verification/tests/affected_verification.rs; crates/cockpit-verification/tests/graph.rs; crates/cockpit-verification/tests/execution.rs; crates/cockpit-verification/tests/route_convergence.rs` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_quick_install_release.py` | implemented-different-by-design | `crates/cockpit-release/src/manifest.rs; crates/cockpit-release/src/archive.rs; crates/cockpit-release/tests/cli.rs; docs/getting-started/security-release-verification.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_readiness_policy.py` | implemented-different-by-design | `crates/cockpit-repository/src/lib.rs; crates/cockpit-cli/tests/doctor.rs; docs/reference/installed-lifecycle.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_real_absurd_injection.py` | implemented-different-by-design | `crates/cockpit-core/tests/adversarial_v2.rs; crates/cockpit-repository/tests/input_trust.rs; tests/adversarial/manifest.json; docs/security/adversarial-validation.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_real_absurd_injection_docs.py` | implemented-different-by-design | `tests/adversarial/README.md; docs/security/adversarial-validation.md; crates/cockpit-repository/tests/input_trust.rs` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_real_adopter_reference_validation.py` | implemented-different-by-design | `tests/release/adopter_acceptance.sh; tests/release/adopter_upgrade_acceptance.sh; crates/cockpit-repository/tests/repository_context.rs; crates/cockpit-cli/tests/agent.rs; docs/reference/installed-lifecycle.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_recovery_usability.py` | implemented-different-by-design | `crates/cockpit-protocol/tests/recovery_outcome.rs; crates/cockpit-repository/tests/recovery_decision.rs; crates/cockpit-repository/src/outcome_render.rs; docs/reference/outcome-report.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_reference_impact.py` | implemented-different-by-design | `crates/cockpit-core/tests/operation_time_policy.rs; crates/cockpit-repository/tests/governance_signals.rs; crates/cockpit-repository/tests/input_trust.rs; docs/reference/operation-time-policy-reevaluation.md; docs/security/adversarial-validation.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_release_archive_contract.sh` | implemented-different-by-design | `crates/cockpit-release/src/archive.rs; crates/cockpit-release/tests/archive.rs; tests/release/source_archive_policy_test.sh; docs/release/distribution.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_release_distribution.py` | implemented-different-by-design | `crates/cockpit-release/src/lib.rs; crates/cockpit-release/src/archive.rs; crates/cockpit-release/src/manifest.rs; crates/cockpit-release/tests/cli.rs; crates/cockpit-release/tests/manifest.rs; docs/release/distribution.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_release_preflight.py` | implemented-different-by-design | `crates/cockpit-release/src/lib.rs; crates/cockpit-release/src/manifest.rs; crates/cockpit-release/tests/manifest.rs; crates/cockpit-release/tests/sbom.rs; crates/cockpit-repository/tests/ci_quality_gate.rs; docs/getting-started/security-release-verification.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_release_state_consistency.py` | implemented-different-by-design | `crates/cockpit-release/src/manifest.rs; crates/cockpit-release/tests/manifest.rs; crates/cockpit-protocol/tests/resource_finalization.rs; docs/release/distribution.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_release_workflow.py` | implemented-different-by-design | `.github/workflows/release.yml; tests/release/workflow_policy.sh; tests/release/adopter_acceptance.sh; tests/release/adopter_upgrade_acceptance.sh; crates/cockpit-release/tests/handoff.rs; docs/release/distribution.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_required_evidence.py` | implemented-different-by-design | `crates/cockpit-repository/src/governance_controls.rs; crates/cockpit-repository/tests/governance_controls.rs; crates/cockpit-verification/tests/policy_planner.rs; docs/reference/verification-semantics.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_rollback.py` | implemented-different-by-design | `crates/cockpit-repository/tests/evolution.rs; crates/cockpit-cli/tests/profile_propose.rs; docs/reference/upgrade.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_ruff016_compatibility.py` | implemented-different-by-design | `tests/ci/repository_gate_manifest_test.py; crates/cockpit-cli/tests/ci_gate.rs; docs/reference/ci-quality-gates.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_schema_migration.py` | implemented-different-by-design | `crates/cockpit-protocol/src/lib.rs; crates/cockpit-repository/src/lib.rs; crates/cockpit-cli/tests/attach.rs; docs/reference/upgrade.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_start_and_archive.py` | implemented-different-by-design | `crates/cockpit-repository/tests/lifecycle_entry.rs; crates/cockpit-repository/tests/lifecycle_order.rs; crates/cockpit-repository/tests/archive_integrity.rs; crates/cockpit-repository/tests/resource_finalization_transition.rs; crates/cockpit-cli/tests/lifecycle.rs; crates/cockpit-cli/tests/worktree_entry.rs` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_supply_chain.py` | implemented-different-by-design | `crates/cockpit-release/src/sbom.rs; crates/cockpit-release/src/archive.rs; crates/cockpit-release/tests/sbom.rs; crates/cockpit-release/tests/archive.rs; crates/cockpit-release/tests/manifest.rs; tests/release/source_archive_policy_test.sh; docs/reference/ci-release-evidence.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_sync_published_release_projection.py` | implemented-different-by-design | `crates/cockpit-release/src/manifest.rs; crates/cockpit-release/src/handoff.rs; crates/cockpit-release/tests/handoff.rs; crates/cockpit-release/tests/manifest.rs; docs/release/distribution.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_system_invariants.py` | implemented-different-by-design | `crates/cockpit-repository/src/governance_controls.rs; crates/cockpit-repository/tests/ci_quality_gate.rs; crates/cockpit-release/tests/manifest.rs; docs/reference/ci-release-evidence.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_task_outcome_ai_finish_integration.py` | implemented-different-by-design | `crates/cockpit-repository/src/lib.rs; crates/cockpit-repository/tests/outcome_report.rs; crates/cockpit-cli/tests/outcome_handoff.rs; crates/cockpit-cli/tests/intelligence.rs` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_task_outcome_generator.py` | implemented-different-by-design | `crates/cockpit-protocol/src/lib.rs; crates/cockpit-repository/src/outcome_render.rs; crates/cockpit-repository/tests/outcome_report.rs; docs/reference/outcome-report.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_task_outcome_markdown_renderer.py` | implemented-different-by-design | `crates/cockpit-repository/src/outcome_render.rs; crates/cockpit-cli/tests/outcome_handoff.rs; docs/reference/outcome-report.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_task_outcome_multilingual.py` | implemented-different-by-design | `crates/cockpit-repository/src/outcome_render.rs; crates/cockpit-cli/tests/intelligence.rs; crates/cockpit-cli/tests/outcome_handoff.rs; docs/reference/multilingual-semantic-parity.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_task_outcome_schema.py` | implemented-different-by-design | `crates/cockpit-protocol/src/lib.rs; crates/cockpit-protocol/tests/intelligence_schema.rs; crates/cockpit-repository/tests/outcome_report.rs; .ai/schemas/task_outcome.schema.json` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_task_outcome_validator.py` | implemented-different-by-design | `crates/cockpit-protocol/src/lib.rs; crates/cockpit-repository/src/outcome_render.rs; crates/cockpit-repository/tests/outcome_report.rs; crates/cockpit-cli/tests/intelligence.rs` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_terminology.py` | implemented-different-by-design | `crates/cockpit-repository/src/outcome_render.rs; docs/reference/governance-profiles.md; docs/reference/multilingual-semantic-parity.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_test_weakening.py` | implemented-different-by-design | `crates/cockpit-repository/tests/ci_quality_gate.rs; crates/cockpit-verification/tests/affected_verification.rs; docs/reference/test-weakening-guard.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_trust_guards.py` | implemented-different-by-design | `crates/cockpit-repository/src/governance_controls.rs; crates/cockpit-repository/tests/input_trust.rs; crates/cockpit-core/tests/operation_time_policy.rs; docs/security/trust-layer.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_trust_layer_demo.py` | implemented-different-by-design | `crates/cockpit-repository/tests/input_trust.rs; docs/security/trust-layer.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_trust_layer_docs.py` | implemented-different-by-design | `docs/security/trust-layer.md; docs/security/trust-layer.zh-CN.md; docs/security/trust-layer.ja.md; tests/docs/documentation_acceptance.sh` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_trust_schema.py` | implemented-different-by-design | `crates/cockpit-protocol/src/lib.rs; crates/cockpit-protocol/tests/protocol.rs; crates/cockpit-repository/tests/contract_schema.rs; docs/security/trust-layer.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_typescript_fixture.py` | reference-only | `docs/reference/reference-file-comparison.md` — Source-specific fixture, wizard, or historical knowledge material; the target records the boundary and does not copy source bytes. |
| `tests/test_uninstall_facts.py` | implemented-different-by-design | `crates/cockpit-agent/src/lib.rs; crates/cockpit-agent/tests/isolation.rs; crates/cockpit-cli/tests/agent.rs; docs/reference/installed-lifecycle.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_uninstall_proposal.py` | implemented-different-by-design | `crates/cockpit-agent/src/lib.rs; crates/cockpit-cli/tests/agent.rs; crates/cockpit-repository/src/lib.rs; docs/reference/installed-lifecycle.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_unknown_confirmation.py` | implemented-different-by-design | `crates/cockpit-repository/src/governance_controls.rs; crates/cockpit-repository/tests/preflight_review.rs; crates/cockpit-protocol/tests/preflight_review.rs; docs/reference/outcome-report.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_upgrade_apply.py` | implemented-different-by-design | `crates/cockpit-repository/src/lib.rs; crates/cockpit-cli/tests/attach.rs; crates/cockpit-repository/tests/evolution.rs; docs/reference/upgrade.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_upgrade_conflict_report.py` | implemented-different-by-design | `crates/cockpit-repository/src/lib.rs; crates/cockpit-cli/tests/attach.rs; docs/reference/upgrade.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_upgrade_proposal.py` | implemented-different-by-design | `crates/cockpit-repository/src/lib.rs; crates/cockpit-cli/tests/attach.rs; crates/cockpit-repository/tests/evolution.rs; docs/reference/upgrade.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_verification_evidence.py` | implemented-different-by-design | `crates/cockpit-protocol/src/lib.rs; crates/cockpit-repository/src/lib.rs; crates/cockpit-repository/tests/evidence_assurance.rs; crates/cockpit-repository/tests/verification_service.rs` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_verification_impact_graph.py` | implemented-different-by-design | `crates/cockpit-verification/src/lib.rs; crates/cockpit-verification/tests/graph.rs; crates/cockpit-verification/tests/affected_verification.rs; docs/reference/verification-cost.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_verification_policy.py` | implemented-different-by-design | `crates/cockpit-verification/tests/policy_planner.rs; crates/cockpit-verification/tests/route_convergence.rs; crates/cockpit-verification/tests/affected_verification.rs; crates/cockpit-repository/tests/project_governance.rs; docs/reference/governance-profiles.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_wizard_fixtures.py` | reference-only | `docs/reference/reference-file-comparison.md` — Source-specific fixture, wizard, or historical knowledge material; the target records the boundary and does not copy source bytes. |
| `tests/test_wizard_io.py` | reference-only | `docs/reference/reference-file-comparison.md` — Source-specific fixture, wizard, or historical knowledge material; the target records the boundary and does not copy source bytes. |
| `tests/test_wizard_localization.py` | reference-only | `docs/reference/reference-file-comparison.md` — Source-specific fixture, wizard, or historical knowledge material; the target records the boundary and does not copy source bytes. |
| `tests/test_work_item_intelligence.py` | implemented-different-by-design | `crates/cockpit-knowledge/src/lib.rs; crates/cockpit-knowledge/tests/query.rs; crates/cockpit-repository/tests/intelligence.rs; crates/cockpit-repository/tests/knowledge_cache.rs; crates/cockpit-cli/tests/intelligence.rs; crates/cockpit-cli/tests/knowledge.rs` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_work_item_intelligence_benchmark.py` | implemented-different-by-design | `crates/cockpit-verification/tests/cost_observation.rs; crates/cockpit-cli/tests/performance.rs; docs/reference/verification-cost.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_work_item_intelligence_integration.py` | implemented-different-by-design | `crates/cockpit-knowledge/src/lib.rs; crates/cockpit-repository/tests/intelligence.rs; crates/cockpit-cli/tests/intelligence.rs; docs/reference/work-item-intelligence-interface.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_work_item_lifecycle_closure.py` | implemented-different-by-design | `crates/cockpit-repository/tests/lifecycle_order.rs; crates/cockpit-repository/tests/resource_finalization_transition.rs; crates/cockpit-repository/tests/archive_integrity.rs; crates/cockpit-cli/tests/lifecycle.rs; crates/cockpit-cli/tests/resource_finalization.rs; crates/cockpit-cli/tests/worktree_entry.rs; docs/reference/work-item-lifecycle-closure.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_work_item_lifecycle_timing.py` | implemented-different-by-design | `crates/cockpit-repository/tests/recovery_events.rs; crates/cockpit-cli/tests/performance.rs; docs/reference/verification-cost.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_work_item_state_machine.py` | implemented-different-by-design | `crates/cockpit-protocol/src/lib.rs; crates/cockpit-repository/tests/lifecycle_order.rs; crates/cockpit-repository/tests/recovery_decision.rs; docs/reference/work-item-state-machine.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `tests/test_workflows.py` | implemented-different-by-design | `.github/workflows/ci.yml; .github/workflows/release.yml; tests/ci/repository_gate_manifest_test.py; tests/release/workflow_policy.sh; crates/cockpit-cli/tests/ci_gate.rs; docs/reference/ci-quality-gates.md` — Portable responsibility is represented by the listed Rust Runtime, repository test, CI/release surface, or reader documentation; source implementation and provider-local wire bytes are not copied. |
| `.ai/knowledge/work-items/dependabot-lockfile-sync-20260826.json` | reference-only | `docs/reference/reference-file-comparison.md` — Source-specific fixture, wizard, or historical knowledge material; the target records the boundary and does not copy source bytes. |
| `.ai/knowledge/work-items/dependabot-ruff-0165-sync-20260902.json` | reference-only | `docs/reference/reference-file-comparison.md` — Source-specific fixture, wizard, or historical knowledge material; the target records the boundary and does not copy source bytes. |

Attached object/adopter repositories inherit the same shared external Runtime,
explicit `--repo` context, isolated Contract/evidence/knowledge, dynamic
