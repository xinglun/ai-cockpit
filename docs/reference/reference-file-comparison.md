...
WI-273 rebaselines only the target checkout metadata and the canonical
comparison snapshot to this reviewed default-branch commit. Existing
classification records, including WI-270/WI-272 and the four explicit
migrate-gap paths, remain unchanged; no deferred path is promoted by a
metadata refresh.
