...
A current Runtime may instead append the deleted sequence-1 transition before
close; in that normal path, close binds the deleted transition head. Promotion
and audit tooling must distinguish these bindings without rewriting either
historical record.
