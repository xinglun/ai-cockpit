...
- [协作不变量覆盖情况](collaboration-invariant-coverage.zh-CN.md)——将十条协作语言不变量映射到既有自动化测试覆盖,引用确切测试,并说明唯一剩余的缺口。
- [协作语言契约](collaboration-language-contract.zh-CN.md)——将交流节点(开始、授权、验证、阻断/恢复、合并、Outcome、交接)映射到既有 Runtime 事实的横向索引。
- [协作场景矩阵](collaboration-scenario-matrix.zh-CN.md)——由状态与转换生成的协作场景矩阵,明确区分实际观测、既有文档与人工构造的案例。
...
- [TypeScript Web fixture 适配](typescript-fixture-adaptation.zh-CN.md)——逐文件的 Rust 原生 TypeScript/Web fixture 映射与明确的安装边界。
- [Mixed-monorepo fixture 适配](mixed-monorepo-fixture-adaptation.zh-CN.md)——不复制工具链、对混合 Python/Node 示例逐文件记录 Rust 原生边界。
