...
生产 CLI 和 MCP 边界使用同一个仅返回适配器，记录分段交接数量并标记
`hostDeliveryMode: "full_handoff_only"`；这只能证明完整正文已交给工具消费者，不能证明下游
Codex 或 Claude 对话已经展示 assistant 消息。本仓库没有 Codex 或 Claude 的发送 API，因此自动
对话投递仍不受支持，并如实标记为 `unknown`。如果宿主适配器提供发送 API，必须调用共享投递函数并返回逐消息确认。
...
发布 Work Item 只有在 Summary 提供明确且绑定证据的 `release` 投影时，两个视图才会展示版本、发布链接、安装验收、升级验收、清理状态和证据引用。
普通 Work Item 不会得到发布章节；没有绑定证据的发布形状 Summary 会保持未知，不会变成发布声明。
