...
- **需要理解的状态**：以上六个节点描述的一切，都应当仅凭 Runtime 本身——`status`、已持久化的当前或归档 Outcome、当前/已归档的 Contract、Summary 与决定记录——就能重新得出，而不依赖任何对话记录。目前没有专门的 Runtime 命令能把"交接状态"打包进一次调用；接手的 Agent 需要从上述同一组 `status`/`outcome`/Contract/Summary 来源自行拼出这份状态。WI-756 的有界检查（`crates/cockpit-cli/tests/collaboration_handoff.rs`）在受控仓库中验证了这一重建，包括已持久化的 `finish.governance` 阻断；它不会把失败命令的瞬时展示当作完成。
...
8. **既有授权是否适用，由规则与记录决定，不能因为更换会话而被任意丢弃或扩大。** 已就 Receipt 记载（[`.ai/glossary.md`](../../.ai/glossary.md)："只有当所有已授权的身份绑定仍然匹配时才可复用"）；并在[第七节](#七agent-或会话交接)中应用于 Agent/会话交接。WI-756 增加了一个有界的新子进程重建检查：它在不创建新决定的前提下，带出已记录的授权与未解决的停止边界；但不宣称模拟所有可能的调用者身份转换。
...
本文档是一份仅由文档结构性检查（frontmatter 完整性、内部链接可解析性、三语言齐备性）与维护者/评审者阅读来验证的纯文档产物——**不是**由自动化语义检查验证，也不是由任何用户研究验证。它并不宣称上述每一条不变量今天都已具备跨入口的自动化强制执行；不变量5在所引 parity 检查之外仍有更窄的入口边界。以下内容被划为独立的、后续的 Work Item，而不是并入本次交付：
...
- 覆盖更多生命周期状态、入口、语言和调用者身份的交接检查；WI-756 是针对已持久化阻断情形的有界受控仓库检查，不宣称穷举覆盖。
