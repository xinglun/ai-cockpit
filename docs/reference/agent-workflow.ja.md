...
- 通常の `start` と `work-item new` は、専用の linked worktree と non-default branch から実行しなければなりません。repository の primary worktree は同期済み default branch 専用です。実装をここに bind すると、finalization で branch/worktree の正確な削除を証明できません。Runtime は Work Item を書く前に primary worktree と既知の default branch を拒否します。明確な remote default base がない linked worktree も拒否されます。これは fail-closed の topology check であり、provider bypass ではありません。
  通常の Work Item を `close` する前にも同じ topology boundary を適用します。primary worktree または検出された default branch にいる場合、Runtime は close decision や cleanup receipt を書く前に拒否します。専用の linked Work Item worktree に戻ってください。これにより不可能な binding を作らず、検証済みの成果を後続の cleanup 用に保持できます。
- `work-item new` も start boundary です。machine receipt と人向け出力には、残った Work Item、取得済みの非デフォルト remote-tracking branch、worktree、cleanup obligation を読むだけの `startAdvisory` が含まれます。remote branch の一覧はローカルに取得済みの ref に基づく観測であり、provider 側の完全な一覧を意味しません。実装前に `warnings` と `nextActions` を確認します。無関係な残留は advisory として継続できますが、exact resource conflict は先に解決しなければなりません。各 lifecycle command の完了後は、返された next action（または `status`）を読み、その action を実行してから次へ進みます。Work Item の成果が完了したことと branch/worktree の cleanup は別の事実です。cleanup の失敗は pending として記録し、完了した成果を未完了へ暗黙に戻してはいけません。
...
- すでに選択された recovery に複数段の successor が含まれる場合は、完全な lineage が揃ってから `work-item recover-selected-successor-lineage` を一度だけ使用します。これは append-only aggregate receipt であり、隣接する各 edge と各 archived node の正確な Contract、Summary、Outcome、Events、verification、archive、close、finalization、provider PR、human-decision evidence を bind しなければなりません。有効な aggregate は対象 node の pending close projection だけを進め、別の successor を作成せず、不完全な履歴を昇格せず、歴史 bytes を置き換えません。missing、stale、foreign、malformed、fork、ambiguous な member は引き続き fail-closed です。
