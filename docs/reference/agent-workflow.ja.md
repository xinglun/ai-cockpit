...
- `work-item new` も start boundary です。machine receipt と人向け出力には、残った Work Item、branch、worktree、cleanup obligation を読むだけの `startAdvisory` が含まれます。実装前に `warnings` と `nextActions` を確認します。無関係な残留は advisory として継続できますが、exact resource conflict は先に解決しなければなりません。各 lifecycle command の完了後は、返された next action（または `status`）を読み、その action を実行してから次へ進みます。Work Item の成果が完了したことと branch/worktree の cleanup は別の事実です。cleanup の失敗は pending として記録し、完了した成果を未完了へ暗黙に戻してはいけません。
