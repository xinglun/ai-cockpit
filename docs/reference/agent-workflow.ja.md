...
するものではありません。
現在の Runtime は `close` 前に deleted sequence-1 transition を append することもあります。この
通常の経路では `close` は deleted transition head を束縛します。Promotion と監査ツールはこの
2 つの束縛を区別し、いずれの履歴 record も書き換えてはなりません。
