...
当 Contract 进入 policy route 时，`resolve_verification_route` 会在 command 执行前
绑定声明的 intent、scenario 名称、required scenario、operation 和 stage。缺少
intent、required scenario coverage 或 operation/stage 不匹配都会 fail closed。该
route 同时供 Agent Risk validation 使用，因此 typed required check、
`agentCapability` 与 `executionDecision` 不会形成只在 CLI 中存在的第二套 policy。
