...
| Historical direct-merge says `resourceContext.<field>` does not match | A receipt identity field differs from the Contract or plan output. | Use the named field in the error, correct the human-owned receipt input, and retry. The Runtime writes no partial record on rejection. |
