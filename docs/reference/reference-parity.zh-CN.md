...
| WI-225——Getting Started parity 登记恢复 | 进行中 | `.ai/evidence/WI-225-getting-started-parity-registration.verification.json`；`.ai/decisions/WI-223-getting-started-parity.recovery.json`；`.ai/decisions/WI-225-getting-started-parity-registration.finalize.json`。仅补三语 parity 登记，不改写 WI-223；pre-merge 关闭等待 hosted checks。 |
