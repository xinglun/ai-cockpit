...
| WI-170——v0.2.20 发布后 parity 与分支收敛 | 进行中 | 本 Work Item 记录发布后文档修正与已验证合并分支的精确清理证据，然后再进入下一批完整对比。 |
| WI-171——收尾链修复 successor | 进行中 | 本 successor 将 WI-170 的不可变 recovery decision 绑定到 close 前缺失的资源收尾边界。 |
