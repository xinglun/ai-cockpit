...
| WI-190——finalization-plan 顺序恢复 | 已实现 | `.ai/evidence/WI-190-finalization-plan-order.verification.json`；`.ai/work-items/archive/WI-190-finalization-plan-order.archive.json`；`.ai/decisions/WI-190-finalization-plan-order.finalize.json`。Runtime 验证的 blocked 回执为 PR #150 记录 `awaiting_merge_close`；在默认分支落地精确 close 或 recovery decision 前，它始终不是终态。 |
| WI-192——治理完整性 finalization 顺序恢复 | 已实现 | `.ai/decisions/WI-188-governance-integrity-gate.recovery.json`；`.ai/evidence/WI-192-governance-integrity-finalization-order.verification.json`；`.ai/work-items/archive/WI-192-governance-integrity-finalization-order.archive.json`；`.ai/decisions/WI-192-governance-integrity-finalization-order.finalize.json`。该 clean successor 保留 v0.2.23 compatibility finding，并在 preflight 与 verification 前绑定准确的 PR resource context；合并前 blocked 回执保持 `awaiting_merge_close`，直到合并后 close。 |
