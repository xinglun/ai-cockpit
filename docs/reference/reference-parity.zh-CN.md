...
| WI-226——Getting Started parity 登记最终恢复 | 进行中 | `.ai/evidence/WI-226-getting-started-parity-registration-final.verification.json`；`.ai/decisions/WI-225-getting-started-parity-registration.recovery.json`；`.ai/decisions/WI-226-getting-started-parity-registration-final.finalize.json`。保留两个 predecessor，同时把 Contract 与 provider receipt 绑定到实际默认分支 base；pre-merge 关闭等待 hosted checks。 |
