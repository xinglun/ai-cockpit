...
| WI-227——capability、Getting Started 与 CI parity 集成 | 进行中 | `.ai/work-items/active/WI-227-getting-started-capability-parity-integration.contract.json`；`.ai/decisions/WI-224-ci-reference-parity.recovery.json`；draft PR #176。这个 bounded integration 保留 WI-222/224 bytes 和 WI-223→WI-225→WI-226 链，并必须在 pre-merge finalization 后再次通过严格 canonical manifest。 |
