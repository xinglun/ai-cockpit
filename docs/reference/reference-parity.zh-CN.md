...
| WI-205——v0.2.25 发布与 transition 兼容性恢复（当前） | 进行中 | [Work Item](../work-items/WI-205-release-v0-2-25.zh-CN.md)；`.ai/evidence/WI-205-release-v0-2-25.verification.json`；`.ai/decisions/WI-205-release-v0-2-25.finalize.json`；在记录正确默认分支 base 与 `finalize-plan` 后，由本 WI 绑定 v0.2.25 公开 Release、adopter acceptance 与安装版 transition 支持。 |
...
| WI-209——Release tag finalization 顺序（当前） | 已实现 | [Work Item](../work-items/WI-209-release-tag-pending-close-finalization.zh-CN.md)；`.ai/evidence/WI-209-release-tag-pending-close-finalization.verification.json`；`.ai/decisions/WI-209-release-tag-pending-close-finalization.finalize.json`；PR #158 在 verification 前绑定同步的默认分支 base，当前等待 merge-close。 |
