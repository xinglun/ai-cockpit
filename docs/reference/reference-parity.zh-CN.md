...
| WI-210——v0.2.26 不可变发布与 adopter 验收（当前） | 已实现 | [Work Item](../work-items/WI-210-release-v0-2-26.zh-CN.md)；`.ai/evidence/WI-210-release-v0-2-26.verification.json`；`.ai/decisions/WI-210-release-v0-2-26.finalize.json`；发布基线与合并前 finalization 已绑定，不可变 tag、公开 adopter/upgrade 和 close 证据仍属于合并后的边界。 |
...
| WI-212——WI-211 收尾恢复（当前） | 进行中 | [Work Item](../work-items/WI-212-release-fixture-finalization-recovery.zh-CN.md)；保留 WI-211 不可变 recovered history，并在 successor verification 前恢复 PR #160 资源收尾；合并关闭仍待完成。 |
