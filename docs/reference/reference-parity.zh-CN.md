...
| WI-222——Runtime 绑定 capability/status 投影（linked immutable delivery） | 进行中 | `.ai/work-items/archive/WI-222-capability-manifest-status.archive.json`；`.ai/evidence/WI-222-capability-manifest-status.verification.json`；`.ai/decisions/WI-222-capability-manifest-status.finalize.json`；commit `4eda504`；PR #173。Runtime v0.2.28 只支持一个 primary successor predecessor，因此 WI-227 Contract 绑定精确 archive/evidence/finalize digests，不伪造第二条 recovery edge。 |
...
| WI-227——capability、Getting Started 与 CI parity 集成 | 进行中 | `.ai/work-items/archive/WI-227-getting-started-capability-parity-integration.archive.json`；`.ai/evidence/WI-227-getting-started-capability-parity-integration.verification.json`；`.ai/decisions/WI-224-ci-reference-parity.recovery.json`；`.ai/decisions/WI-227-getting-started-capability-parity-integration.finalize.json`；draft PR #176。这个 bounded integration 保留 WI-222/224 bytes 和 WI-223→WI-225→WI-226 链，并在通过 finalization 后的严格 canonical manifest 后继续等待 merge-close。 |
...
| WI-230——v0.2.29 发布基线 | 进行中 | `.ai/work-items/archive/WI-230-release-v0-2-29.archive.json`；`.ai/evidence/WI-230-release-v0-2-29.verification.json`；`.ai/decisions/WI-230-release-v0-2-29.finalize.json`；预发布基线承载已合并的 WI-229 Runtime 修复，并绑定已审阅 PR 后再进入不可变发布。public Release、adopter、upgrade 与已安装 Runtime 验收属于发布后的独立 successor 范围。 |
