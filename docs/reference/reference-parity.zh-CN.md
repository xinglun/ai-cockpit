...
| WI-208——Release tag finalization 顺序（当前） | 已实现 | [Work Item](../work-items/WI-208-release-tag-pending-close-finalization.zh-CN.md)；`.ai/evidence/WI-208-release-tag-pending-close-finalization.verification.json`；`.ai/decisions/WI-208-release-tag-pending-close-finalization.finalize.json`；PR #158 的准确上下文在 verification 前绑定，当前等待 merge-close。 |
