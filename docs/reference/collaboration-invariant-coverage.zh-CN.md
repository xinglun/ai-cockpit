...
title: "协作不变量覆盖情况"
description: "将十条协作语言语义不变量映射到既有的自动化测试覆盖，引用确切的测试文件与函数，并准确说明仍存在的部分覆盖限制。"
...
- 目前十条不变量都至少具有直接或部分自动化覆盖;仍存在的部分限制已
  在第3行和第10行明确说明。

## 覆盖更新与剩余限制

**不变量7(下一步的正确性)**现在已由 WI-741 提供有界的直接覆盖。测试将
SCN-001、SCN-002 和 SCN-016 的下一步文本与场景矩阵记录的
`expected.keyMessage` 进行比对,并守护这些场景仍被观测且继续覆盖不变量7。
它不声称为 `docs/reference/collaboration-scenario-matrix.json` 中的每个
状态建立穷尽判定器。

本表中不再有未测试的不变量被列为已命名缺口。剩余限制是第3行和第10行
明确说明的部分覆盖。
