...
| WI-192 — governance-integrity finalization-order recovery | Implemented | `.ai/decisions/WI-188-governance-integrity-gate.recovery.json`; `.ai/evidence/WI-192-governance-integrity-finalization-order.verification.json`; `.ai/work-items/archive/WI-192-governance-integrity-finalization-order.archive.json`; `.ai/decisions/WI-192-governance-integrity-finalization-order.finalize.json`. This clean successor preserves the v0.2.23 compatibility finding and binds the exact pull-request resource context before preflight and verification; its blocked pre-merge receipt remains `awaiting_merge_close` until merge and close. |
