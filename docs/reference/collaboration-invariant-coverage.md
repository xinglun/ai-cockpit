...
bounded executable checks tied to observed Runtime states. The checks are not
a universal oracle for every state/option combination. Section V's separate
handoff-completeness check remains independently scoped so that its evidence
can prove conversation-free reconstruction without being conflated with the
state/transition consistency check here.
