...
現在の installation baseline は公開済みで identity-bound な `v0.2.33` Release です。Homebrew と manual install は
...
`v0.2.33` も検証します。`tests/ci/runtime_verify_shadow.sh` receipt は standard/strict route の **execution smoke**
です。identity-bound public `v0.2.33` を検証し、repository の canonical profile を実行します。
...
v0.2.33 の checksum file は全十個の archive/SBOM を対象にするため、download した archive だけを検証します。
...
なまま保持し、成功した Release として再標識しません。v0.2.33 の公開後は bytes が immutable
...
v0.2.33 は同じ schema の patch Release ですが、N-1 run は同じ harness を使い、compatibility
...
この fallback は現在公開済みの identity-bound な `v0.2.33` tag で利用できます。Workspace は複数 package を含むため `cockpit-cli` を明示します。

```bash
cargo install --git https://github.com/xinglun/ai-cockpit.git --tag v0.2.33 --locked --root "$HOME/.local" --bin ai-cockpit cockpit-cli
"$HOME/.local/bin/ai-cockpit" --version
