...
公開後の installation baseline は identity-bound な `v0.2.73` Release です。provider Release が存在する前は、直前の公開
`v0.2.72` archive を使います。予約済みの `v0.2.51` tag は immutable な公開失敗試行
...
`v0.2.73` も検証します。`tests/ci/runtime_verify_shadow.sh` receipt は standard/strict route の **execution smoke**
です。identity-bound public `v0.2.73` を検証し、repository の canonical profile を実行します。
...
git tag -a v0.2.73 -m 'ai-cockpit v0.2.73'
test "$(git cat-file -t v0.2.73)" = tag
test "$(git rev-parse v0.2.73^{})" = "$(git rev-parse HEAD)"
...
v0.2.73 の checksum file は全十個の archive/SBOM を対象にするため、download した archive だけを検証します。

```bash
archive="ai-cockpit-v0.2.73-aarch64-apple-darwin.tar.gz"
...
```bash
archive="ai-cockpit-v0.2.73-aarch64-apple-darwin.tar.gz"
gh release download v0.2.73 --repo xinglun/ai-cockpit \
...
なまま保持し、成功した Release として再標識しません。v0.2.73 の公開後は bytes が immutable
...
v0.2.73 は同じ schema の patch Release ですが、N-1 run は同じ harness を使い、compatibility
...
archive="ai-cockpit-v0.2.73-${target}.tar.gz"
expected="$(awk -v name="$archive" '$2 == name {print $1}' SHA256SUMS)"
actual="$(shasum -a 256 "$archive" | awk '{print $1}')"
test -n "$expected" && test "$expected" = "$actual"
...
```powershell
$archive = "ai-cockpit-v0.2.73-x86_64-pc-windows-msvc.zip"
...
この fallback は現在公開済みの identity-bound な `v0.2.73` tag で利用できます。Workspace は複数 package を含むため `cockpit-cli` を明示します。

```bash
cargo install --git https://github.com/xinglun/ai-cockpit.git --tag v0.2.73 --locked --root "$HOME/.local" --bin ai-cockpit cockpit-cli
"$HOME/.local/bin/ai-cockpit" --version
