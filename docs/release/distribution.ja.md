...
現在の installation baseline は公開済みで identity-bound な `v0.2.38` Release です。Homebrew と manual install は
...
`v0.2.38` も検証します。`tests/ci/runtime_verify_shadow.sh` receipt は standard/strict route の **execution smoke**
です。identity-bound public `v0.2.38` を検証し、repository の canonical profile を実行します。
...
v0.2.38 の checksum file は全十個の archive/SBOM を対象にするため、download した archive だけを検証します。

```bash
archive="ai-cockpit-v0.2.38-aarch64-apple-darwin.tar.gz"
...
```bash
archive="ai-cockpit-v0.2.38-aarch64-apple-darwin.tar.gz"
gh release download v0.2.38 --repo xinglun/ai-cockpit \
...
なまま保持し、成功した Release として再標識しません。v0.2.38 の公開後は bytes が immutable
...
v0.2.38 は同じ schema の patch Release ですが、N-1 run は同じ harness を使い、compatibility
...
archive="ai-cockpit-v0.2.38-${target}.tar.gz"
expected="$(awk -v name="$archive" '$2 == name {print $1}' SHA256SUMS)"
actual="$(shasum -a 256 "$archive" | awk '{print $1}')"
test -n "$expected" && test "$expected" = "$actual"
...
```powershell
$archive = "ai-cockpit-v0.2.38-x86_64-pc-windows-msvc.zip"
...
この fallback は現在公開済みの identity-bound な `v0.2.38` tag で利用できます。Workspace は複数 package を含むため `cockpit-cli` を明示します。

```bash
cargo install --git https://github.com/xinglun/ai-cockpit.git --tag v0.2.38 --locked --root "$HOME/.local" --bin ai-cockpit cockpit-cli
"$HOME/.local/bin/ai-cockpit" --version
