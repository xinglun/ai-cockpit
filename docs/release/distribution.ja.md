...
現在の installation baseline は公開済みで immutable な `v0.2.4` Release です。Homebrew と manual install は
...
v0.2.4 は同じ schema の patch Release ですが、N-1 run は同じ harness を使い、compatibility
...
この fallback は現在公開済みの immutable な `v0.2.4` tag で利用できます。Workspace は複数 package を含むため `cockpit-cli` を明示します。
