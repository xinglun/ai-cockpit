...
現在の installation baseline は公開済みで identity-bound な `v0.2.48` Release です。Homebrew と manual install は
...
`v0.2.48` も検証します。`tests/ci/runtime_verify_shadow.sh` receipt は standard/strict route の **execution smoke**
です。identity-bound public `v0.2.48` を検証し、repository の canonical profile を実行します。
...
v0.2.48 の checksum file は全十個の archive/SBOM を対象にするため、download した archive だけを検証します。
...
なまま保持し、成功した Release として再標識しません。v0.2.48 の公開後は bytes が immutable
...
v0.2.48 は同じ schema の patch Release ですが、N-1 run は同じ harness を使い、compatibility
...
この fallback は現在公開済みの identity-bound な `v0.2.48` tag で利用できます。Workspace は複数 package を含むため `cockpit-cli` を明示します。
