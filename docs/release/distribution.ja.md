...
現在の installation baseline は公開済みで immutable な `v0.2.21` Release です。Homebrew と manual install は
...
```bash
archive="ai-cockpit-v0.2.21-aarch64-apple-darwin.tar.gz"
...
```bash
archive="ai-cockpit-v0.2.21-aarch64-apple-darwin.tar.gz"
gh release download v0.2.21 --repo xinglun/ai-cockpit \
...
**v0.2.21 の完全な adopter acceptance baseline は `x86_64-unknown-linux-gnu` です。**
...
v0.2.21 は同じ schema の patch Release ですが、N-1 run は同じ harness を使い、compatibility
...
archive="ai-cockpit-v0.2.21-${target}.tar.gz"
expected="$(awk -v name="$archive" '$2 == name {print $1}' SHA256SUMS)"
actual="$(shasum -a 256 "$archive" | awk '{print $1}')"
test -n "$expected" && test "$expected" = "$actual"
...
```powershell
$archive = "ai-cockpit-v0.2.21-x86_64-pc-windows-msvc.zip"
...
この fallback は現在公開済みの immutable な `v0.2.21` tag で利用できます。Workspace は複数 package を含むため `cockpit-cli` を明示します。

```bash
cargo install --git https://github.com/xinglun/ai-cockpit.git --tag v0.2.21 --locked --root "$HOME/.local" --bin ai-cockpit cockpit-cli
"$HOME/.local/bin/ai-cockpit" --version
