...
WI-239 に永続化された provider snapshot と現在の provider API は、この Release を
`immutable: false` と報告します。したがって Release identity は provider-immutable ではなく
drift-detectable です。tag、release-manifest、`SHA256SUMS`、archive digest、post-release
receipt が一致し続ける必要があります。
...
公開済みで identity-bound な Release、対象 repository path、OS に合う archive が必要です。Homebrew install
...
```bash
archive="ai-cockpit-v0.2.31-aarch64-apple-darwin.tar.gz"
gh release download v0.2.31 --repo xinglun/ai-cockpit \
...
repository に保持された WI-239 receipt が durable な public-binary adopter baseline です。
GitHub Actions run `32696048024` では `x86_64-unknown-linux-gnu` の staged、public、N-1
adopter path も完了しましたが、この hosted Linux artifact は外部かつ provider retention
対象の短期 evidence であり、repository に永続化された baseline ではありません。他の公開 target
は build/smoke evidence のみで、別の acceptance receipt が永続化されない限り full adopter
lifecycle の完了とは主張しません。

### 過去の N-1 schema migration 受入れ

schema が変わった基準は、過去の v0.1.1 から v0.2.0 への migration です。
v0.2.31 は同じ schema の patch Release ですが、N-1 run は同じ harness を使い、compatibility
...
archive="ai-cockpit-v0.2.31-${target}.tar.gz"
expected="$(awk -v name="$archive" '$2 == name {print $1}' SHA256SUMS)"
actual="$(shasum -a 256 "$archive" | awk '{print $1}')"
test -n "$expected" && test "$expected" = "$actual"
...
```powershell
$archive = "ai-cockpit-v0.2.31-x86_64-pc-windows-msvc.zip"
...
この fallback は現在公開済みで identity-bound な `v0.2.31` tag で利用できます。Workspace は複数 package を含むため `cockpit-cli` を明示します。
...
Rollback では、名前付きの過去 Release archive の manifest と digest を verify してから binary を
