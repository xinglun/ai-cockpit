...
当前发布产物命名为 `ai-cockpit-v0.2.89-<target>.tar.gz`（Windows 使用对应的
archive）。只有公开 Release 和发布后验收完成后，它才成为安装基线。

发布完成后，当前安装基线是公开且绑定身份的 `v0.2.89` Release；在 provider Release
...
v0.2.89 的校验文件覆盖全部十个 archive/SBOM，因此只校验实际下载的 archive：
...
Release。失败且未公开的 v0.2.77 tag 仅作为历史保留。v0.2.89 发布后，公开 bytes 才成为不可变事实；其 `SHA256SUMS` 覆盖五个 archive 与五个
...
发生 schema 变化的基线是历史上的 v0.1.1 到 v0.2.0 迁移。v0.2.89 是保持同一
...
该 fallback 适用于当前已发布且绑定身份的 `v0.2.89` tag。
