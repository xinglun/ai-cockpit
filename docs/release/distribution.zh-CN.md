...
当前安装基线是公开且不可变的 `v0.2.26` Release。Homebrew 和手动安装都使用公开 archive
...
source-quality 失败；两者都没有公开 Release。它们是不可变历史，不是安装基线；`v0.2.26` 是下一候选版本。
...
**v0.2.26 的完整 adopter acceptance 基线为 `x86_64-unknown-linux-gnu`。**
...
发生 schema 变化的基线是历史上的 v0.1.1 到 v0.2.0 迁移。v0.2.26 是保持同一
...
该 fallback 适用于当前已发布的不可变 `v0.2.26` tag。
