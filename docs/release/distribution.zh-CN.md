...
当前安装基线是公开且不可变的 `v0.2.7` Release。Homebrew 和手动安装都使用公开 archive
...
**v0.2.7 的完整 adopter acceptance 基线为 `x86_64-unknown-linux-gnu`。**
...
发生 schema 变化的基线是历史上的 v0.1.1 到 v0.2.0 迁移。v0.2.7 是保持同一
...
该 fallback 适用于当前已发布的不可变 `v0.2.7` tag。
