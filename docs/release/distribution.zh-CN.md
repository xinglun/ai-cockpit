...
发布完成后，当前安装基线是公开且绑定身份的 `v0.2.65` Release；在 provider Release
...
`v0.2.65`。`tests/ci/runtime_verify_shadow.sh` receipt 是 standard/strict route 的 **execution
smoke**。它验证公开且绑定身份的 `v0.2.65`，并使用仓库规范 profile。它不宣称 Runtime
...
v0.2.65 的校验文件覆盖全部十个 archive/SBOM，因此只校验实际下载的 archive：
...
Release。v0.2.65 发布后，公开 bytes 才成为不可变事实；其 `SHA256SUMS` 覆盖五个 archive 与五个
...
发生 schema 变化的基线是历史上的 v0.1.1 到 v0.2.0 迁移。v0.2.65 是保持同一
...
该 fallback 适用于当前已发布且绑定身份的 `v0.2.65` tag。
