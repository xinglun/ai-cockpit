...
当前安装基线是公开且绑定身份的 `v0.2.35` Release。Homebrew 和手动安装都使用公开 archive
...
`v0.2.34` 标签作为失败的发布历史保留（workflow run `33155382717`）；它没有公开
Release 或可安装制品，也不是安装基线。
...
`v0.2.35`。`tests/ci/runtime_verify_shadow.sh` receipt 是 standard/strict route 的 **execution
smoke**。它验证公开且绑定身份的 `v0.2.35`，并使用仓库规范 profile。它不宣称 Runtime
...
v0.2.35 的校验文件覆盖全部十个 archive/SBOM，因此只校验实际下载的 archive：
...
Release。v0.2.35 发布后，公开 bytes 才成为不可变事实；其 `SHA256SUMS` 覆盖五个 archive 与五个
...
**v0.2.35 adopter acceptance 基线：`aarch64-apple-darwin`。**
发布后的 acceptance successor 会在发布后持久化 v0.2.35 的公开 binary receipt；仓库保留的 WI-239 receipt
...
发生 schema 变化的基线是历史上的 v0.1.1 到 v0.2.0 迁移。v0.2.35 是保持同一
...
该 fallback 适用于当前已发布且绑定身份的 `v0.2.35` tag。
