...
发布完成后，当前安装基线是公开且绑定身份的 `v0.2.60` Release；在 provider Release
存在之前，应使用上一个公开的 `v0.2.59` archive。预留的 `v0.2.51` tag 是不可变的发布失败尝试
...
此前公开的 `v0.2.59` 基线继续作为历史 evidence 保留，不会复用为当前安装 identity；`v0.2.55`、`v0.2.53` 与 `v0.2.52` 也继续作为更早的历史基线保留。未公开的 `v0.2.49` 标签（workflow run `33379366308`）作为发布前失败的不可变历史保留，不是安装基线。
...
`v0.2.60`。`tests/ci/runtime_verify_shadow.sh` receipt 是 standard/strict route 的 **execution
smoke**。它验证公开且绑定身份的 `v0.2.60`，并使用仓库规范 profile。它不宣称 Runtime
...
v0.2.60 的校验文件覆盖全部十个 archive/SBOM，因此只校验实际下载的 archive：
...
Release。v0.2.60 发布后，公开 bytes 才成为不可变事实；其 `SHA256SUMS` 覆盖五个 archive 与五个
...
发生 schema 变化的基线是历史上的 v0.1.1 到 v0.2.0 迁移。v0.2.60 是保持同一
...
该 fallback 适用于当前已发布且绑定身份的 `v0.2.60` tag。
