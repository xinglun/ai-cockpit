...
当前候选发布产物命名为 `ai-cockpit-v0.2.99-<target>.tar.gz`（Windows 使用对应的
archive）。只有公开 Release 和发布后验收完成后，它才成为安装基线。

当前安装基线目标是 `v0.2.99` 候选；只有公开 Release 和发布后验收完成后才会成为公开基线。在此之前，公开且绑定身份的 `v0.2.94` Release 仍是已安装基线；在其 provider Release 存在之前，应使用上一个公开的 `v0.2.93` archive。`v0.2.77` tag 作为没有 provider Release 的不可变发布失败历史保留，不能作为安装基线。预留的 `v0.2.51` tag 是不可变的发布失败尝试
