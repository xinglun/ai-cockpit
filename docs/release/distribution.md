...
The current release artifact naming is `ai-cockpit-v0.2.89-<target>.tar.gz` or
...
The public, identity-bound `v0.2.89` Release is the current installation baseline
after publication; before the provider Release exists, use the preceding public
`v0.2.85` archive. The `v0.2.84` tag is retained as immutable failed-publication
...
The failed `v0.2.88` tag is also retained as immutable publication-failure
history (WI-764, workflow run `34371183927`); it has no provider Release and
must never be reused or treated as an installation baseline.
...
The `v0.2.68` tag is retained as immutable failed-publication history (workflow
run `33795945667`): source quality failed before a provider Release was created,
so it is not an installation baseline and must never be reused.
...
The previous public `v0.2.72` baseline remains historical evidence after this
release and is not reused as the current installation identity. The earlier
public `v0.2.63`, `v0.2.61`, `v0.2.55`, `v0.2.53`, and `v0.2.52` baselines also remain historical evidence. The unpublished `v0.2.49` tag
...
current release route additionally verifies `v0.2.85`. The
`tests/ci/runtime_verify_shadow.sh` receipt is an **execution smoke** for
standard/strict routes. It verifies identity-bound public `v0.2.85` and runs the
...
published GitHub Release. The v0.2.89 checksum file covers all ten archive/SBOM
...
unpublished v0.2.77 tag is likewise retained only as history. For v0.2.89,
...
v0.2.89 is a same-schema patch release: its N-1 run follows the same harness
...
This fallback is available for the current identity-bound `v0.2.89` tag.
...
cargo install --git https://github.com/xinglun/ai-cockpit.git \
  --tag v0.2.89 --locked --root "$HOME/.local" \
