...
The public, identity-bound `v0.2.77` Release is the current installation baseline
after publication; before the provider Release exists, use the preceding public
`v0.2.76` archive. The reserved `v0.2.51` tag is an immutable failed publication
...
current release route additionally verifies `v0.2.77`. The
`tests/ci/runtime_verify_shadow.sh` receipt is an **execution smoke** for
standard/strict routes. It verifies identity-bound public `v0.2.77` and runs the
...
published GitHub Release. The v0.2.77 checksum file covers all ten archive/SBOM
...
remains immutable and is not relabeled as a successful Release. For v0.2.77,
...
v0.2.77 is a same-schema patch release: its N-1 run follows the same harness
...
This fallback is available for the current identity-bound `v0.2.77` tag.
...
cargo install --git https://github.com/xinglun/ai-cockpit.git \
  --tag v0.2.77 --locked --root "$HOME/.local" \
