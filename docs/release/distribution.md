...
The public immutable `v0.2.25` Release is the current installation baseline.
...
and has no public Release. It is immutable history, not an installation
baseline; the next candidate is `v0.2.25`.
...
**Complete adopter acceptance baseline: `x86_64-unknown-linux-gnu` for v0.2.25.**
...
v0.2.25 is a same-schema patch release: its N-1 run follows the same harness
...
This fallback is available for the current immutable `v0.2.25` tag.
...
cargo install --git https://github.com/xinglun/ai-cockpit.git \
  --tag v0.2.25 --locked --root "$HOME/.local" \
