...
published GitHub Release. The v0.2.31 checksum file covers all ten archive/SBOM
...
v0.2.31 is a same-schema patch release: its N-1 run follows the same harness
...
cargo install --git https://github.com/xinglun/ai-cockpit.git \
  --tag v0.2.31 --locked --root "$HOME/.local" \
