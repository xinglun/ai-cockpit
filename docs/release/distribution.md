...
The public, identity-bound `v0.2.35` Release is the current installation baseline.
...
The `v0.2.34` tag is retained as failed publication history (workflow run
`33155382717`); it has no public Release or installable artifact and is not an
...
current release route additionally verifies `v0.2.35`. The
`tests/ci/runtime_verify_shadow.sh` receipt is an **execution smoke** for
standard/strict routes. It verifies identity-bound public `v0.2.35` and runs the
...
published GitHub Release. The v0.2.35 checksum file covers all ten archive/SBOM
...
remains immutable and is not relabeled as a successful Release. For v0.2.35,
...
**Adopter acceptance baseline for v0.2.35: `aarch64-apple-darwin`.**
The post-release acceptance successor persists the v0.2.35 public-binary receipt after publication; the
...
v0.2.35 is a same-schema patch release: its N-1 run follows the same harness
...
This fallback is available for the current identity-bound `v0.2.35` tag.
...
cargo install --git https://github.com/xinglun/ai-cockpit.git \
  --tag v0.2.35 --locked --root "$HOME/.local" \
