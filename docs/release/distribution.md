...
current release route additionally verifies `v0.2.78`. The
`tests/ci/runtime_verify_shadow.sh` receipt is an **execution smoke** for
standard/strict routes. It verifies identity-bound public `v0.2.78` and runs the
...
published GitHub Release. The v0.2.78 checksum file covers all ten archive/SBOM
...
unpublished v0.2.77 tag is likewise retained only as history. For v0.2.78,
...
v0.2.78 is a same-schema patch release: its N-1 run follows the same harness
...
This fallback is available for the current identity-bound `v0.2.78` tag.
...
cargo install --git https://github.com/xinglun/ai-cockpit.git \
  --tag v0.2.78 --locked --root "$HOME/.local" \
