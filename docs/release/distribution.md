...
The public immutable `v0.2.26` Release is the current installation baseline.
...
and the immutable `v0.2.25` tag records a later source-quality failure; neither
has a public Release. They are immutable history, not installation baselines;
`v0.2.26` is the next release candidate.
...
**Complete adopter acceptance baseline: `x86_64-unknown-linux-gnu` for v0.2.26.**
...
v0.2.26 is a same-schema patch release: its N-1 run follows the same harness
...
This fallback is available for the current immutable `v0.2.26` tag.
...
cargo install --git https://github.com/xinglun/ai-cockpit.git \
  --tag v0.2.26 --locked --root "$HOME/.local" \
