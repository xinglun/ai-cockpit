...
The public immutable `v0.2.5` Release is the current installation baseline.
...
v0.2.5 is a same-schema patch release: its N-1 run follows the same harness
...
This fallback is available for the current immutable `v0.2.5` tag.
...
cargo install --git https://github.com/xinglun/ai-cockpit.git \
  --tag v0.2.5 --locked --root "$HOME/.local" \
