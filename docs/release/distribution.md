...
The public, identity-bound `v0.2.65` Release is the current installation baseline
after publication; before the provider Release exists, use the preceding public
`v0.2.63` archive. The reserved `v0.2.51` tag is an immutable failed publication
...
The previous public `v0.2.63` baseline remains historical evidence after this
release and is not reused as the current installation identity. The earlier
public `v0.2.61`, `v0.2.55`, `v0.2.53`, and `v0.2.52` baselines also remain historical evidence. The unpublished `v0.2.49` tag
...
current release route additionally verifies `v0.2.65`. The
`tests/ci/runtime_verify_shadow.sh` receipt is an **execution smoke** for
standard/strict routes. It verifies identity-bound public `v0.2.65` and runs the
...
published GitHub Release. The v0.2.65 checksum file covers all ten archive/SBOM
...
remains immutable and is not relabeled as a successful Release. For v0.2.65,
...
v0.2.65 is a same-schema patch release: its N-1 run follows the same harness
...
This fallback is available for the current identity-bound `v0.2.65` tag.
...
cargo install --git https://github.com/xinglun/ai-cockpit.git \
  --tag v0.2.65 --locked --root "$HOME/.local" \
