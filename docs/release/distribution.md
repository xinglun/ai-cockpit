...
The public immutable `v0.2.11` Release is the current installation baseline.
...
**Complete adopter acceptance baseline: `x86_64-unknown-linux-gnu` for v0.2.11.**
...
v0.2.11 is a same-schema patch release: its N-1 run follows the same harness
...
This fallback is available for the current immutable `v0.2.11` tag.
...
cargo install --git https://github.com/xinglun/ai-cockpit.git \
  --tag v0.2.11 --locked --root "$HOME/.local" \
