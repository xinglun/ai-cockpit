...
The current release artifact naming is `ai-cockpit-v0.2.83-<target>.tar.gz` or
...
The public, identity-bound `v0.2.83` Release is the current installation baseline
after publication; before the provider Release exists, use the preceding public
`v0.2.82` archive. The `v0.2.77` tag is retained as immutable failed-publication
...
current release route additionally verifies `v0.2.82`. The
`tests/ci/runtime_verify_shadow.sh` receipt is an **execution smoke** for
standard/strict routes. It verifies identity-bound public `v0.2.82` and runs the
...
published GitHub Release. The v0.2.83 checksum file covers all ten archive/SBOM
...
unpublished v0.2.77 tag is likewise retained only as history. For v0.2.83,
...
v0.2.83 is a same-schema patch release: its N-1 run follows the same harness
...
This fallback is available for the current identity-bound `v0.2.83` tag.
...
cargo install --git https://github.com/xinglun/ai-cockpit.git \
  --tag v0.2.83 --locked --root "$HOME/.local" \
