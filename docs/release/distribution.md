...
The public, identity-bound `v0.2.43` Release is the current installation baseline.
...
current release route additionally verifies `v0.2.43`. The
`tests/ci/runtime_verify_shadow.sh` receipt is an **execution smoke** for
standard/strict routes. It verifies identity-bound public `v0.2.43` and runs the
...
published GitHub Release. The v0.2.43 checksum file covers all ten archive/SBOM
...
remains immutable and is not relabeled as a successful Release. For v0.2.43,
...
v0.2.43 is a same-schema patch release: its N-1 run follows the same harness
...
This fallback is available for the current identity-bound `v0.2.43` tag.
...
cargo install --git https://github.com/xinglun/ai-cockpit.git \
  --tag v0.2.43 --locked --root "$HOME/.local" \
