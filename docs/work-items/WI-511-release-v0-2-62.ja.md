...
title: "WI-511 — v0.2.62 release と public adopter acceptance"
...
# WI-511 — v0.2.62 release と public adopter acceptance

[English](WI-511-release-v0-2-62.md) · [简体中文](WI-511-release-v0-2-62.zh-CN.md)

## Intent

review 済み main から v0.2.62 を公開し、隔離した adopter と N-1 acceptance
で immutable public artifact を検証します。その binary をこの repository
へ install してから、次の reference-source 比較 batch を再開します。

## Scope

- workspace package、lockfile、現在の三言語 release/versioning guidance を v0.2.62 にそろえ、過去の事実は保持します。
...
- downloaded immutable artifact だけで public adopter と N-1 acceptance を実行し、isolation、evidence binding、not_ready scaffold、temporary-root cleanup proof を含めます。
...
1. workspace package と lockfile が v0.2.62 を示し、現在の三言語 release guidance が過去の release history を書き換えず更新される。
2. synchronized main から annotated v0.2.62 tag を作成する前に、review 済み PR の hosted checks がすべて成功する。
3. public Release が identity-bound で byte-consistent な archive、SHA256 checksum、SBOM、provenance、release manifest を公開する。
