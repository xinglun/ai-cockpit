...
`docs_governance_integrity` に失敗した。WI-775 は recovered predecessor として保持し、
WI-776 が最新 default branch からこの projection の archive-evidence redelivery を完了する
bounded successor である。WI-773 の immutable Contract、benchmark evidence、Outcome、
finalization、close、candidate 却下判断は変更しない。

## Scope

- WI-774 の archive、verification evidence、recovery decision、hosted failure binding を
  保持し、historical bytes は書き換えない。
- ordered redelivery は WI-775 → WI-776 が担当し、parity registration を fresh verification
  evidence より先に commit する。
