...
[English](WI-574-release-v0-2-75.md) · [日本語](WI-574-release-v0-2-75.ja.md)

# WI-574——v0.2.75 发布与公开制品验收

## 目标

从已审查并同步的默认分支发布 v0.2.75，形成不可变 Runtime 基线；随后证明下载的公开
binary 可以在不使用源码 checkout 或 workspace fallback 的情况下治理本仓库。

## 范围与边界

- 在三种语言中同步 Cargo 元数据、lockfile、发布与版本说明至 v0.2.75。
...
1. Cargo 元数据、lockfile 和当前发布/版本页面标识 v0.2.75，同时保留 v0.2.74 作为前一个公开基线。
2. Release CI 为 v0.2.75 生成绑定 identity 的五目标制品和供应链证据集。
3. public adopter 与 N-1 验收只使用下载的 v0.2.75 制品，证明隔离和清理，并使用同一 binary
   验证本仓库。
4. 发布从同步且 ready 的默认分支开始，不改 Runtime 行为、对象工程、全局配置或无关历史证据。
5. 发布或 adopter 验收失败时保留不可变发布事实并记录失败 receipt，不回退到源码 checkout 或 workspace binary。
