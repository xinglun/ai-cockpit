...
description: "Performance、Outcome、HCI、architecture convergence の evidence-based acceptance report。"
audience:
  - adopter
  - contributor
  - maintainer
...
- **HCI (C):** 三言語の first Work Item route は `start --prepare` から始まり、reviewable、mergeable、closed を区別し、provider/release detail は詳細 workflow reference に置く。
...
| HCI/default path | 三言語 semantic/documentation gate | `tests/docs/getting_started_semantic.sh` | pending |
