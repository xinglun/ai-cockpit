...
reviewed GitHub PR #318。archive 与 evidence 保持为历史 bytes。archived retry 的独立缺陷
通过明确的 recovery receipt
`.ai/decisions/WI-353-runtime-recovery-delivery-binding.recovery.json` 继续由 WI-355 处理。
...
- reviewed delivery 已记录 locked workspace tests、formatting check 与 clippy，并将 predecessor 标记为已恢复。
- PR resource context 绑定到 [PR #318](https://github.com/xinglun/ai-cockpit/pull/318)，其 base
  为 `main`/`origin`，并使用专用 recovery worktree。
- fresh 的 archived-retry 修复、verification、Provider finalization、精确 branch/worktree
  清理和结构化 close 由 WI-355 负责。本文件不改写 predecessor archive，也不把 successor
  工作声称为 predecessor 的历史结果。
