...
title: "WI-565 — v0.2.73 release と公開 artifact acceptance"
...
[English](WI-565-release-v0-2-73.md) · [简体中文](WI-565-release-v0-2-73.zh-CN.md)

# WI-565 — v0.2.73 release と公開 artifact acceptance

## Objective

review 済み default branch から v0.2.73 を immutable Runtime baseline として公開し、
...
1. Cargo metadata、lockfile、current release/versioning page が v0.2.73 を示し、v0.2.72 を直前の public baseline として保持する。
2. Release CI が identity-bound な五 target artifact と supply-chain receipt 一式を生成する。
3. Public adopter と N-1 acceptance は v0.2.73 の download artifact だけを使い、isolation と cleanup を証明し、同じ binary でこの repository を検証する。
