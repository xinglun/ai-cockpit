...
| A prior broad user authorization is repeatedly presented as a new preflight confirmation after snapshot changes. | The previous receipt bound `preflightDecisionDigest=sha256:1c6980110d6452fa8ff819865ad28ba9c0d175200442ec13f0e38346fde8e63f`, while the latest recorded preflight digest was `sha256:87984c0a1a26d0916db4f2c1b13f031eb89189e9b24d41497391ad35de9cf55e`; Runtime requested another `confirm_review` despite unchanged user authority through merge and exact cleanup, stopping before release. An append-only identity-bound rebind restored green preflight without asserting verification or review. | Separate persistent bounded authority from snapshot-specific evidence. Permit automatic append-only rebinding only while Work Item, Contract authority/scope, base, and stop boundary remain identical; require a new human decision when any of those change. Never treat the rebind as verification or review evidence. |
...
| The hosted governance gate can surface missing lineage/parity inputs that are not obvious from the active worktree. | CI run `36155139121` diagnostic artifact `repository-gate-diagnostics/docs_governance_integrity.log` reported WI-1031 missing its terminal close decision, stale parity status in all three locales, and missing Japanese parity rows for WI-1032 and WI-1033. | Before expensive hosted gates, compare the exact staged candidate against a clean checkout and enumerate every required lineage receipt and locale parity row. Preserve missing close evidence as a blocker; never synthesize it. |
| Platform-specific test dependencies can remain hidden until hosted CI compiles a different target. | In CI run `36155139121`, `windows-runtime` failed `cargo test --locked -p cockpit-repository --lib` because `tempfile` was configured only for Unix while Windows library tests also use it. | Add a target-matrix dependency/compile preflight that catches test-only dependency gaps before PR CI, while retaining the Windows job as authoritative execution evidence. |
