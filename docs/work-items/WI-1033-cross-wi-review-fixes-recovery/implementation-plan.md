...
| Platform-specific dependencies and types can remain hidden until hosted CI compiles a different target. | Run `36155139121` first found `tempfile` missing from Windows library tests; run `36174008746` then found a Windows-only `u32`/`i32` comparison and an unconditional Unix `CString` import in the process-identity code. | Add a target-matrix dependency/compile preflight that catches test-only dependency and cfg-specific type/import gaps before PR CI, while retaining the Windows job as authoritative execution evidence. |
