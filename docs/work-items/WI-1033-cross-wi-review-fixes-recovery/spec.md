...
| retired_work_item_requires_all_locale_parity_rows | A valid retired/replaced Work Item passes only when all required English, Simplified Chinese, and Japanese rows reference the retirement receipt and state not_verified; removing any locale row or all rows fails closed with missing_parity_entry. | Run the governance-integrity gate with valid three-locale rows, then remove each locale row separately and all rows; require missing_parity_entry and no successful verification claim. |
