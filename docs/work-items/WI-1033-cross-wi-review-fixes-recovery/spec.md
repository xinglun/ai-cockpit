...
18. Coordination-request transitions validate both the request ID and stored target Work Item ID as safe path components before resolving request or registration paths, require the loaded registration identity to match that target, and reject mismatched stored request IDs without changing bytes.
...
| coordination_request_identity_is_path_safe | Traversal in either the request ID or stored target Work Item ID is rejected; a registration record whose embedded Work Item ID differs from the stored target is also rejected before request bytes change. | Attempt traversal IDs against valid request/registration-shaped records outside their expected directories, mutate a stored requestId, and point a valid target path at a registration with a different embedded Work Item ID; assert errors and byte-for-byte unchanged request records. |
