...
title: "WI-509——发布 v0.2.61 与公开 adopter 验收"
...
# WI-509——发布 v0.2.61 与公开 adopter 验收

[English](WI-509-release-v0-2-61.md) · [日本語](WI-509-release-v0-2-61.ja.md)

## 意图

从经过审查的 main 发布 v0.2.61，在继续参考源逐文件比对前，用隔离的
adopter 验收证明不可变公开发布 artifact。

## 范围

- 将 workspace package、lockfile 和当前三语发布/版本文档对齐到 v0.2.61，保留历史事实。
...
- 仅使用下载的不可变 artifact 执行 public adopter 与 N-1 验收，包含隔离、evidence 绑定、not_ready scaffold 与清理证明。
...
1. workspace package 与 lockfile 标识 v0.2.61；当前三语发布文档更新且不改写历史发布记录。
2. 在同步 main 创建 annotated v0.2.61 tag 前，审查过的 PR hosted checks 全部通过。
3. 公开 Release 提供绑定身份的 archive、SHA256 校验和、SBOM、provenance 与 release manifest。
