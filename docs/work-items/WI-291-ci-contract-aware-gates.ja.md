...
description: "hosted stale-parity rejection 後の immutable failed delivery を保持し、WI-292 が同じ bounded batch を再配信する。"
audience:
  - maintainer
  - reviewer
status: recovered
...
を拒否しました。lifecycle bytes は immutable に保持し、WI-292 が明示的な
...
- 最新の remote default base からの再配信は WI-292 のみが行う。
...
と provider lifecycle は WI-292 が担当します。
