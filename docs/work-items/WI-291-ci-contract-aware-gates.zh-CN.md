...
description: "保留 hosted stale-parity 阻断后的不可变失败交付；WI-292 与 WI-293 保留连续恢复历史。"
audience:
  - maintainer
  - reviewer
status: recovered
...
WI-292 与 WI-293 是显式 successor，不改写本次尝试。

## 边界

- 原样保留 WI-291 archive、verification、阻断的 finalization 与 recovery 记录。
- 不把失败 PR 视为已合并或已发布。
- 仅由 successor 链（WI-292、再到 WI-293）从最新远端默认分支重新交付同一实现。
