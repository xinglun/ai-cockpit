...
[English](WI-565-release-v0-2-73.md) · [日本語](WI-565-release-v0-2-73.ja.md)

# WI-565——v0.2.73 发布与公开产物验收

## 目标

从已审查的默认分支发布 v0.2.73，形成不可变 Runtime 基线；随后证明从公开
...
1. Cargo 元数据、lockfile 和当前发布/版本页面标识 v0.2.73；v0.2.72 保留为紧邻的上一个公开基线。
2. Release CI 生成绑定身份的五目标产物及供应链回执集合。
3. 公开 adopter 和 N-1 验收只使用 v0.2.73 下载产物，证明隔离与清理，并用同一 binary 验证本仓库。
