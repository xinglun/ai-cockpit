...
description: Establish an order-preserving, evidence-bound performance benchmark before any runtime optimization.
...
[简体中文](WI-662-p0-benchmark-evidence.zh-CN.md) · [日本語](WI-662-p0-benchmark-evidence.ja.md)
...
- 原始样本、预热次数、样本数、分位数算法、首次 CLI、OS 缓存预热后的独立 CLI 和常驻 MCP 边界均明确记录；样本不足时不声明可靠 p95/p99。
- baseline/candidate 可使用不同 Runtime 身份，但各自证据、仓库快照和环境必须完整且可比较；既有 schema 1 负向 gate 覆盖保留。
- 阶段耗时、场景矩阵、实际读取/哈希字节、Git 调用、进程和峰值内存不可可靠取得时显式标记不可用，不填零。
...
见 Contract 和计划文件中的 focused tests、documentation acceptance、parity check、治理 gate 及 `git diff --check`。完成前不得把此 Work Item 标记为 verified 或 closed。
