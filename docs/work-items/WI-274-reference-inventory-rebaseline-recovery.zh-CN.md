...
title: "WI-274——参考源 inventory rebaseline 恢复"
workItemId: WI-274-reference-inventory-rebaseline-recovery
description: "在保留 WI-273 不可变失败事实的同时，从同步后的默认分支重新交付逐文件参考源 inventory。"
audience:
  - maintainer
  - reviewer
status: in_progress
...
# WI-274——参考源 inventory rebaseline 恢复

## 意图

在同步默认分支 `origin/main@487f01970c49e2b85d17b0cb0536f9d60c8f05e0` 上重新建立逐文件、机器可读的参考源比对基线。WI-273 因 parity registration 与 verification evidence 在同一提交中，无法证明顺序，已作为不可变失败事实保留。

## 范围

- 将 inventory 元数据、路径摘要和文档计数重新绑定到同步后的默认分支。
- 记录 WI-273 的不可变失败边界并链接 successor。
- 在任何生成的 verification evidence 之前提交 parity registration。
- 保持英文、中文、日文比对和 parity 文档同步。

## 边界

本恢复不改写 WI-273 历史、不放宽治理门、不增加 Runtime 行为、不改变 CI 架构，也不执行延期的架构清洁；仅处理参考源 inventory 比对批次。

## 验收

- inventory 元数据和路径摘要匹配固定目标提交。
- WI-273 不可变 evidence 与 successor 关系可审计。
