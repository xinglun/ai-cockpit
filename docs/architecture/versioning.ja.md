...
直前の公開 patch release `v0.2.74` は historical evidence として保持します。`v0.2.65`、`v0.2.63`、`v0.2.61`、`v0.2.55`、`v0.2.53`、`v0.2.52` もさらに前の歴史として保持します。予約済みの `v0.2.51` tag は
workflow run `33417057474` の immutable な lightweight-tag 公開失敗で、provider Release はなく再利用しません。未公開で失敗した
`v0.2.49` tag は再利用せず、現在の package identity は `v0.2.75` です。
