...
当前发布候选是 `v0.2.100`；在公开 Release 和发布后验收完成之前，它不是安装基线。当前不可变公开基线仍是发布后的 `v0.2.94`；在其 provider Release 存在之前，`v0.2.93` 仍是可安装的公开基线。
...
`v0.2.56` 标签同样是不可变的发布失败历史：source-quality 在创建 provider Release 之前失败，因此永远不会复用或作为安装基线。
未公开的 `v0.2.36` 标签保留为不可变的 staged 验收失败历史，不是安装基线。
此前公开的 `v0.2.72` Release 继续作为历史 evidence 保留，并由当前基线取代；`v0.2.65`、`v0.2.63`、`v0.2.61`、`v0.2.55`、`v0.2.53` 与 `v0.2.52` 作为更早历史保留。
失败的 `v0.2.49` 标签继续作为不可变的发布前历史保留，没有公开 Release。
失败的 `v0.2.46` 标签（`33330269507`）作为不可变发布历史保留；由于打 tag 前遗漏了已关闭
Work Item 的文档 promotion，它没有创建公开 Release，也不是安装基线。

失败的 `v0.2.84` 标签因 source-quality gate 在创建 provider Release 前失败而作为不可变未公开历史保留，永远不能复用或作为安装基线。
...
- 上一个不可变公开基线是 `v0.2.94`；当前发布基线目标是 `v0.2.100` 候选，public adopter acceptance 和 N-1 升级验收仍属于发布后 evidence。外部 Homebrew tap 是
