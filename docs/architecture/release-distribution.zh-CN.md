...
发布完成后当前不可变发布基线是 `v0.2.89`；在其 provider Release 存在之前，`v0.2.85` 仍是可安装的公开基线。
...
- 当前不可变公开基线是 `v0.2.89`；public adopter acceptance 和 N-1 升级验收属于发布后 evidence。外部 Homebrew tap 是
