...
发布完成后当前不可变发布基线是 `v0.2.53`；在其 provider Release 存在之前，`v0.2.52` 仍是可安装的公开基线。
...
此前公开的 `v0.2.52` Release 继续作为历史 evidence 保留，并由当前基线取代；`v0.2.50` 作为更早历史保留。
...
- 当前不可变公开基线是 `v0.2.53`；public adopter acceptance 和 N-1 升级验收属于发布后 evidence。外部 Homebrew tap 是
