...
当前不可变发布基线是 `v0.2.32`。不可变的 `v0.2.30` 标签因清洁批次发布路由缺陷而作为失败发布历史保留，不是安装基线。
...
- 当前不可变公开基线是 `v0.2.32`；public adopter acceptance 和 N-1 升级验收属于发布后 evidence。外部 Homebrew tap 是
