...
当前发布候选是 `v0.2.97`；在公开 Release 和发布后验收完成之前，它不是安装基线。当前不可变公开基线仍是发布后的 `v0.2.94`；在其 provider Release 存在之前，`v0.2.93` 仍是可安装的公开基线。
...
- 上一个不可变公开基线是 `v0.2.94`；当前发布基线目标是 `v0.2.97` 候选，public adopter acceptance 和 N-1 升级验收仍属于发布后 evidence。外部 Homebrew tap 是
