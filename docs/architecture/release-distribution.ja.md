...
失敗した `v0.2.46` tag（`33330269507`）は不変の公開履歴として保持します。closed Work Item の
ドキュメント promotion が tag 前に不足していたため公開 Release はなく、installation baseline ではありません。

`v0.2.68` tag は immutable な公開失敗履歴として保持します（workflow run `33795945667`）。source quality が provider Release 作成前に失敗したため、installation baseline ではなく再利用しません。

失敗した `v0.2.84` tag は source-quality gate で停止した immutable な未公開履歴として保持し、再利用も installation baseline 化もしません。
...
- 既存の immutable public baseline は `v0.2.113` です。public adopter acceptance と N-1 upgrade 受入れは完了しています。`v0.2.106` は直前の公開 Release として historical evidence に保持します。
