...
現在の immutable release baseline は `v0.2.31` です。immutable な `v0.2.30` tag は clean-batch の route defect による公開失敗履歴として保持し、installation baseline にはしません。
...
- 現在の immutable public baseline は `v0.2.31` で、public adopter acceptance と N-1 upgrade 受入れは post-release evidence です。
