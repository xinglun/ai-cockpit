...
現在の immutable release baseline は `v0.2.33` です。失敗した staged `v0.2.32` tag は WI-299 の finalize binding defect による公開失敗履歴として保持し、公開 Release はなく、installation baseline にはしません。immutable な `v0.2.30` tag も clean-batch の route defect による公開失敗履歴として保持します。
...
- 現在の immutable public baseline は `v0.2.33` で、public adopter acceptance と N-1 upgrade 受入れは post-release evidence です。
