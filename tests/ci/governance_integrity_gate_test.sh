...
# to a self-consistent head value copied into the receipt. Any later code or
# test commit must force a fresh finalization rather than remain accepted.
...
# append separate so the finalization receipt can bind the exact reviewed
# checkout it observes.
receipt_path = repo / ".ai/decisions" / f"{work_item}.finalize.json"
receipt_path.unlink()
subprocess.run(["git", "add", "-A"], cwd=repo, check=True)
subprocess.run(
    ["git", "commit", "-qm", "archive before finalization"], cwd=repo, check=True
)
receipt = json.loads(
