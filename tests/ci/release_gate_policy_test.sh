...
require 'source_target="$source_repo/target"' 'source validation must define a repository-local evidence root'
require 'cp "$GITHUB_WORKSPACE/target/release-quality-route.json" "$source_target/release-quality-route.json"' 'source validation must copy the route receipt inside the immutable source repository'
...
require 'source tests/release/release_close_state.sh' 'close state must use the shared fail-closed reducer'
require 'record_release_close_failure' 'close receipt failures must set the final state to failed'
require 'finalize_release_close_state' 'close receipt writing must enforce failureRoots to failed-state invariant'
...
require 'gh run download "$SOURCE_RUN_ID"' 'close-only recovery must download prior acceptance evidence instead of rerunning it'
