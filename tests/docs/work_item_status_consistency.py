...
def parity_statuses(repository: Path) -> tuple[dict[str, list[str]], list[str]]:
    ...
    for relative, vocabulary in PARITY_DOCUMENTS:
        ...
        for line in path.read_text(encoding="utf-8").splitlines():
            ...
            if len(cells) < 2 or cells[1] not in vocabulary:
                continue
            ...
            if work_item in seen:
                ...
                continue
            seen.add(work_item)
            rows.setdefault(work_item, []).append(vocabulary[cells[1]])
    return rows, errors
...
def check(repository: Path) -> list[str]:
    ...
    for english in sorted(work_item_directory.glob("WI-*.md")):
        ...
        if not (
            valid_close(close, work_item_id, repository_id)
            or valid_recovery(recovery, work_item_id, repository_id)
        ):
            continue
        ...
        if (
            ...
            and valid_recovery(recovery, work_item_id, repository_id)
