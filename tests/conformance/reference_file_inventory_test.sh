...
test "$(jq '[.records[] | select(.batch == "WI-306-reference-file-comparison-batch-04")] | length' "$manifest")" -eq 4
test "$(jq '[.records[] | select(.batch == "WI-306-reference-file-comparison-batch-04" and .classification == "implemented-different-by-design" and (.rustCounterparts | length) > 0 and (.reason | length) > 0)] | length' "$manifest")" -eq 3
test "$(jq -r '.records[] | select(.referencePath == "docs/assets/ai-cockpit-demo.gif") | .classification' "$manifest")" = "reference-only"
test "$(jq '[.records[] | select(.batch == "WI-306-reference-file-comparison-batch-04" and .classification == "deferred-next-batch")] | length' "$manifest")" -eq 0
