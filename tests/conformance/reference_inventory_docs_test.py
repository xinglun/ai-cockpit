...
def read_marker(document: Path) -> dict[str, int]:
    ...
    if len(matches) != 1:
        raise ValueError(f"{document}: expected exactly one inventory count marker")
    values: dict[str, int] = {}
    for field in matches[0].split():
        key, separator, value = field.partition("=")
        if not separator or not value.isdigit():
            raise ValueError(f"{document}: malformed inventory count field: {field}")
...
def main() -> int:
    ...
    for document in documents:
        ...
        if actual != expected:
            raise ValueError(f"{document}: inventory counts {actual} != manifest {expected}")
