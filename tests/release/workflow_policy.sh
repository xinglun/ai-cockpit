...
require_match "jq -er '.plan.request.sourceRevision'" 'release preflight must read the immutable source revision from the canonical ReleasePlan envelope'
require_match "jq -er '.plan.request.baseRevision'" 'release preflight must read the source baseline from the canonical ReleasePlan envelope'
require_match "jq -r '.plan.request.sourceContractPath // .plan.request.contractPath // empty'" 'release preflight must read the source Contract from the canonical ReleasePlan envelope'
...
require_match 'if \[\[ -n "\$INPUT_CONTRACT_PATH" \]\]; then' 'optional Contract input must not be passed to the resolver when empty'
require_match 'if \[\[ -n "\$INPUT_HANDOFF_RUN_ID" \]\]; then' 'optional handoff input must not be passed to the resolver when empty'
