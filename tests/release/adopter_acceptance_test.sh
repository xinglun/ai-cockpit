...
resume_test="$(cd "$(dirname "$0")" && pwd)/adopter_acceptance_resume_test.sh"
...
grep -q -- 'release manifest commit does not match source checkout HEAD' "$script"
...
grep -q -- '--authority-source release-adopter-acceptance' "$script"
...
RUSTUP_HOME="$regression_root/invalid-rustup-home" TMPDIR="$toolchain_tmp" "$script" \
  --repository xinglun/ai-cockpit --tag v0.2.23 --target aarch64-apple-darwin \
  --output "$toolchain_output" --source-repo "$repo" >/dev/null 2>&1
toolchain_exit=$?
set -e
[[ "$toolchain_exit" -eq 1 ]] || { printf 'invalid RUSTUP_HOME must fail closed\n' >&2; exit 1; }
[[ -z "$(find "$toolchain_tmp" -mindepth 1 -maxdepth 1 -type d -name 'ai-cockpit-adopter-acceptance.*' -print -quit)" ]] || {
  printf 'pre-toolchain failure left a run_root behind\n' >&2
  exit 1
}
