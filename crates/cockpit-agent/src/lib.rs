...
const AGENT_FIRST_START_DOCUMENT: &str =
    include_str!("../../../docs/reference/agent-first-start.md");
...
/// Return the canonical, provider-neutral procedure an Agent must read before
/// its first repository operation.  The same text is projected into every
/// repository-owned adapter so the CLI and installed instructions cannot
/// drift.
pub fn first_start_procedure() -> &'static str {
    agent_first_start_body()
}
...
fn agent_first_start_body() -> &'static str {
    AGENT_FIRST_START_DOCUMENT
        .split_once("\n---\n")
        .map(|(_, body)| body.trim())
        .unwrap_or(AGENT_FIRST_START_DOCUMENT.trim())
}
