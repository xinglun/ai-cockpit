...
/// Facts gathered for one `checkpoint_work_item` request: the Contract, the
/// Git snapshot, and the digests derived from them. Gathering these is the
/// Observation phase; `checkpoint_governance_checks` (Governance) and the
/// remaining body of `checkpoint_work_item` (Evidence/Projection) consume
/// this struct without re-reading the same facts.
...
/// Checkpoint's own freshness/ordering validation over already-gathered
/// facts, plus the existing (unchanged) call into the shared preflight
/// governance decision. `require_green_or_yellow_preflight_governance` still
/// performs its own observation internally; only `checkpoint_work_item`'s own
/// body is reorganized here.
