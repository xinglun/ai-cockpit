...
impl ObservationContext {
    ...
    pub(crate) fn contract_model_digest(&self) -> Option<&Digest> {
        self.contract_model_digest.as_ref()
    }
...
fn contract_model_digest(path: &Path) -> Result<Digest, ObserverError> {
    ...
    let normalized = serde_json::to_value(contract).map_err(|error| ObserverError::State {
        path: path.to_path_buf(),
        message: error.to_string(),
    })?;
