...
pub fn render_human_outcome(root: &Path, outcome: &OutcomeV2, language: &str) -> String {
    // Preserve the existing repository API as the complete audit projection;
    // user-facing CLI/MCP adapters choose Summary explicitly by default.
    render_full_outcome(root, outcome, language)
}
...
pub fn render_human_outcome_with_view(
    ...
    match view {
        OutcomeRenderView::Summary => render_summary_outcome(root, outcome, language),
        OutcomeRenderView::Full => render_full_outcome(root, outcome, language),
    }
}

/// Render the complete evidence-oriented handoff explicitly.
pub fn render_full_human_outcome(root: &Path, outcome: &OutcomeV2, language: &str) -> String {
    render_human_outcome_with_view(root, outcome, language, OutcomeRenderView::Full)
}

fn render_summary_outcome(root: &Path, outcome: &OutcomeV2, language: &str) -> String {
    ...
    let raw_lifecycle = lifecycle_status(root, outcome, historical, superseded);
...
fn render_full_outcome(root: &Path, outcome: &OutcomeV2, language: &str) -> String {
